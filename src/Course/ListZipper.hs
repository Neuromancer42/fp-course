...
findLeft p (ListZipper l x r) =
  let (l', l'') = break p l in
      case l'' of
          (y :. ys) -> IsZ $ ListZipper ys y (reverse l' ++ (x :. r))
          _ -> IsNotZ
...
findRight p (ListZipper l x r) =
  let (r', r'') = break p r in
      case r'' of
          (y :. ys) -> IsZ $ ListZipper (reverse r' ++ (x :. l)) y ys
          _ -> IsNotZ
...
moveLeftLoop (ListZipper Nil x r) =
  case reverse (x :. r) of
      y :. ys -> ListZipper ys y Nil
      _ -> ListZipper Nil x Nil
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end z = case moveRightN (length $ rights z) z of
            IsZ z' -> z'
            IsNotZ -> z
...
start ::
  ListZipper a
  -> ListZipper a
start z = case moveLeftN (length $ lefts z) z of
              IsZ z' -> z'
              IsNotZ -> z
...
deletePullLeft (ListZipper (y :. ys) _ r) = IsZ $ ListZipper ys y r
...
deletePullRight (ListZipper l _ (y :. ys)) = IsZ $ ListZipper l y ys
...
-- /Tip:/ Use `zipWith`
  ListZipper fl fx fr <*> ListZipper l x r =
    ListZipper (zip' fl l) (fx x) (zip' fr r)
      where zip' = zipWith (\f t -> f t)
...
instance Extend ListZipper where
  f <<= z =
      ListZipper (unfoldr genL z) (f z) (unfoldr genR z)
    where
      genL zz = case moveLeft zz of
                   IsZ z' -> Full (f z', z')
                   _ -> Empty
      genR zz = case moveRight zz of
                   IsZ z' -> Full (f z', z')
                   _ -> Empty
