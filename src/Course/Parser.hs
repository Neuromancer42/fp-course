...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f pa = P $ \inp ->
  case parse pa inp of
      Result inp' x -> Result inp' (f x)
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser b pa = P $ \inp ->
  case parse pa inp of
      Result inp' x -> parse (b x) inp'
      ErrorResult err -> ErrorResult err
...
list1 ::
  Parser a
  -> Parser (List a)
list1 pa = flbindParser pa $ \x ->
           flbindParser (list pa) $ \xs ->
           valueParser (x :. xs)
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) $ \s ->
          case read s of
...
sequenceParser (p :. ps) = flbindParser p $ \x ->
                           flbindParser (sequenceParser ps) $ \xs ->
                           valueParser $ x :. xs
...
surnameParser = flbindParser upper $ \c ->
                flbindParser (thisMany 5 lower) $ \s ->
                flbindParser (list lower) $ \ss ->
                valueParser $ c :. (s ++ ss)
...
phoneParser ::
  Parser Chars
phoneParser = flbindParser digit $ \s ->
...
personParser ::
  Parser Person
personParser = flbindParser ageParser $ \a ->
               flbindParser spaces1 $ \_ ->
               flbindParser firstNameParser $ \fname ->
               flbindParser spaces1 $ \_ ->
               flbindParser surnameParser $ \sname ->
               flbindParser spaces1 $ \_ ->
               flbindParser smokerParser $ \smoke ->
               flbindParser spaces1 $ \_ ->
               flbindParser phoneParser $ \p ->
               valueParser $ Person a fname sname smoke p
